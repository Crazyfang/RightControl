...
namespace DapperExtensions.SqlServerExt
{
    public static class SqlServerExt
    {
        ...
        public static IEnumerable<T> GetByPageUnite<T>(this IDbConnection conn, int pageIndex, int pageSize, out long total, string returnFields = null, string where = null, object param = null, string orderBy = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            sb.AppendFormat(
                "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY CreateOn desc) AS rownum, * FROM (SELECT {0} FROM {1}) test) AS items WHERE items.rownum BETWEEN {2} AND {3};",
                returnFields, where, skip, skip + pageSize);
        ...
        public static IEnumerable<T> GetByPageInforSearch<T>(this IDbConnection conn, Type t, int pageIndex, int pageSize, out long total, string returnFields = null, string where = null, object param = null, string orderBy = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            DapperExtSqls sqls = GetDapperExtSqls(t);
            if (returnFields == null)
                returnFields = sqls.AllFields;
            ...
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("DECLARE @total INT;SELECT @total = COUNT(1) FROM InforSearchOperate a" +
                            " left join t_user b"+
                            " on a.OperaterId = b.Id"+
                            " left join t_DepartmentUser c"+
                            " on b.Id = c.UserId"+
                            " left join t_department d"+
                            " on c.DepartmentId = d.Id {0};SELECT @total;", where);
            sb.Append("IF(@total>0) BEGIN ");
            if (pageIndex == 1)
            {
                sb.AppendFormat("SELECT TOP ({0}) a.*, b.RealName as OperaterName, d.DepartmentName FROM InforSearchOperate a"+
                                " left join t_user b"+
                                " on a.OperaterId = b.Id"+
                                " left join t_DepartmentUser c"+
                                " on b.Id = c.UserId"+
                                " left join t_department d"+
                                " on c.DepartmentId = d.Id {1} ", pageSize, orderBy);
            }
            else
            {
                sb.AppendFormat("WITH cte AS (SELECT ROW_NUMBER() OVER({1}) AS rownum, a.*, b.RealName as OperaterName, d.DepartmentName "+ 
                                "FROM InforSearchOperate a " + 
                                "left join t_user b " +
                                "on a.OperaterId = b.Id " +
                                "left join t_DepartmentUser c " +
                                "on b.Id = c.UserId " +
                                "left join t_department d " +
                                "on c.DepartmentId = d.Id {0} )", where, orderBy);
                //if (returnFields.Contains(" AS") || returnFields.Contains(" as"))
                //{
                //    sb.AppendFormat("SELECT * FROM cte WHERE cte.rownum BETWEEN {0} AND {1}", skip + 1, skip + pageSize);
                //}
        ...
        public static IEnumerable<T> GetByPageRecordExpired<T>(this IDbConnection conn, Type t, int pageIndex, int pageSize, out long total, string returnFields = null, string where = null, object param = null, string orderBy = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            DapperExtSqls sqls = GetDapperExtSqls(t);
            if (returnFields == null)
                returnFields = sqls.AllFields;
            ...
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("DECLARE @total INT;SELECT @total = COUNT(distinct a.RecordID) FROM RecordTable a " +
                            "left join RecordList b " + 
                            "on a.RecordID = b.RecordID "+
                            "left join OtherFileListTable c "+
                            "on a.RecordID = c.RecordID "+
                            "{0};SELECT @total;", where);
            sb.Append("IF(@total>0) BEGIN ");
            if (pageIndex == 1)
            {
                sb.AppendFormat("SELECT distinct TOP ({0}) a.* from RecordTable a "+
                                "left join RecordList b "+
                                "on a.RecordID = b.RecordID "+
                                "left join OtherFileListTable c "+
                                "on a.RecordID = c.RecordID "+
                                "where b.ExpirationTime < GETDATE() or c.ExpirationTime < GETDATE() {1} ", pageSize, orderBy);
            }
            else
            {
                sb.AppendFormat("with cte as ("+
                                "SELECT distinct a.* from RecordTable a "+
                                "left join RecordList b "+
                                "on a.RecordID = b.RecordID "+
                                "left join OtherFileListTable c "+
                                "on a.RecordID = c.RecordID "+
                ...
                //    sb.AppendFormat("SELECT {0} FROM cte WHERE cte.rownum BETWEEN {1} AND {2}", returnFields, skip + 1, skip + pageSize);
                //}
                sb.AppendFormat("select a.* from (SELECT ROW_NUMBER() OVER(order by RecordID) AS rownum, * from cte) a "+
                                "where a.rownum BETWEEN {0} AND {1}", skip + 1, skip + pageSize);
            }
            sb.Append(" END");
