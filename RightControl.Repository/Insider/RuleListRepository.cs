...
{
    public class RuleListRepository : BaseRepository<RuleList>, IRuleListRepository
    {
        /// <summary>
        /// 获取规则列表
        /// </summary>
        ...
        public IEnumerable<RuleList> GetRule()
        {
            ...
            {
                var sql = "SELECT R_No,R_Name FROM RuleList";
                return conn.Query<RuleList>(sql);
            }
        }
        ...
        public IEnumerable<RuleList> GetRuleName(int no)
        {
            using (var conn = SqlHelper.SqlConnection())
            {
                var sql = "SELECT R_No,R_Name FROM RuleList Where R_No=" + no;
                return conn.Query<RuleList>(sql);
            }
        }
    }
